...
@section('content')
    ...
    <div class="card card-body mb-3">
        ...
        <div class="d-flex">
            <button id="like" class="btn ml-auto" @if(!auth()->check()) disabled @endif>Like <span class="count"></span></button>
    ...
    <script>
        ...
        function checkIsFavorited(el) {
            axios.post( '{{ route('posts.favorites.is_favorited', $post) }}' ).then(function (res) {
                let is_favorited = res.data['is_favorited'];

                if (is_favorited) {
                    el.innerHTML = "Избранное (да)";
                    el.classList.remove('btn-secondary');
                    el.classList.add('btn-danger');
                } else {
                    el.innerHTML = "Избранное (нет)";
                    el.classList.add('btn-secondary');
                    el.classList.remove('btn-danger');
                }
            });
        ...
        favoriteBtn.addEventListener('click', function () {
            ...
            axios.post( '{{ route('posts.favorite', $post) }}' ).then(function (response) {
                let data = response.data;

                if (data.status === 200) {
                    checkIsFavorited(favoriteBtn);
                }
